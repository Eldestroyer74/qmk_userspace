...
/* Graphical bongocat animation, driven by key press timer or WPM.
   It stores right-aligned frames and mirrors them for the left OLED.
...
#define OLED_TRANSFORM_NONE     0
#define OLED_TRANSFORM_MIRROR_X 1
#define OLED_RAW_WIDTH          128
#define OLED_RAW_HEIGHT         32

static void write_frame_byte(uint16_t cursor, uint8_t byte, uint8_t transform) {
	if (transform & OLED_TRANSFORM_MIRROR_X) {
		cursor = (cursor & 0x0180) | (127 - (cursor & 0x007f));
	}
	oled_write_raw_byte(byte, cursor);
}

static void decode_frame_transformed(unsigned char const *frame, uint8_t transform) {
	uint16_t cursor = 0;
	...
	while (i < size) {
		...
		if (count & 0x80) {
			...
			for (uint8_t uniqs = 0; uniqs < count; ++uniqs) {
				...
				write_frame_byte(cursor++, byte, transform);
			}
		} else {
...
void animate_cat(void) {
	...
	if (chieftaindots_cat_state == CAT_BIG_PRESS) {
		...
		decode_frame_transformed(paws, is_keyboard_left() ? OLED_TRANSFORM_MIRROR_X : OLED_TRANSFORM_NONE);
	} else {
		idle_index = idle_index < IDLE_FRAMES - 1 ? idle_index + 1 : 0;
		decode_frame_transformed(idle[idle_index], is_keyboard_left() ? OLED_TRANSFORM_MIRROR_X : OLED_TRANSFORM_NONE);
	}
}
